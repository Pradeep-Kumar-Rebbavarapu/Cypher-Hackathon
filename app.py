...
# ---------------------------
# CSV Upload endpoint (Batch Processing)
# ---------------------------
@app.post("/predict")
...
# ---------------------------
# NEW: Row-by-Row Processing with Streaming
# ---------------------------
@app.post("/predict-stream")
async def predict_stream_endpoint(file: UploadFile = File(...)):
    """
    Upload a CSV file and process rows one by one with streaming response.
    Returns progress updates as each row is processed.
    """
    if not file.filename.endswith('.csv'):
        raise HTTPException(status_code=400, detail="Only CSV files are accepted")
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    yield json.dumps({
                        ...
                        "row_number": idx + 1,
                        "total_rows": total_rows,
                        "progress_percent": round((idx + 1) / total_rows * 100, 2),
                        "data": result
                    }) + "\n"
                except Exception as e:
                    yield json.dumps({
...
# ---------------------------
# NEW: Single Row Processing Endpoint
# ---------------------------
...
async def predict_single_row(row_data: SingleRowInput):
    """
    Process a single turbine data row sent as JSON.
    Useful for real-time processing or testing individual rows.
    """
    try:
...
def root():
    return {
        ...
        "version": "3.0",
        "endpoints": {
            "/predict": "POST - Upload CSV file (batch processing - all rows at once)",
            "/predict-stream": "POST - Upload CSV file (streaming - row by row with progress)",
            "/predict-single": "POST - Send single row as JSON for immediate processing",
